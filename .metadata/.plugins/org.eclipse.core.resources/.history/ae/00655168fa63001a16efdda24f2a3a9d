...
void *exit_state(Person *p, Display *display);

void *start_state(Person *p){
	//Check for scan state, no message made as display creates it
	if(p->state == SCAN_STATE) return scan_state;
...
void *scan_state(Person *p, Display *display){
	...
	//Remove person state so not to trigger display msg on Scan state TODO:does this work in c? did this numerous times
	p->state = NULL;
	return scan_state;
}
...
//Do we need this?? They all just break out when state is set to exit in inputs
void *exit_state(Person *p, Display *display){
...
int main(int argc, char* argv[]) {
	...
	while(1) {
		if((rcvid = MsgReceive(chid, &person, sizeof( person ), NULL)) < 0) {return EXIT_FAILURE;}
		...
		if(person.state == EXIT_STATE)break;
